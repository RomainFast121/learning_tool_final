...
\maketitle
Initial resources: Social 45, Financial 45, Eco 45. Thresholds: low at 30, high at 70.
...
\textbf{Note:} This question is about what kind of uncertainty you are willing to carry into the projectâs early momentum.
\begin{itemize}
...
\item Push for more volume early because scale will reduce the practical importance of the archiveâs unevenness.
\end{itemize}
...
\textbf{Extra / note:} <h3>Reusable method</h3><p>Before signing, make a one-page âpressure mapâ with the actors who gain from speed, the actors who absorb reputational damage if things go wrong, and the specific moments when the team could still slow down. If that map is vague, the agreement is probably more constraining than it looks.</p>\\
...
\begin{itemize}
\item \textbf{Choice 1:} The sponsor mostly solves the projectâs political risk, so the main task now is keeping the momentum high.
\begin{itemize}
\item \textbf{Impact:} No resource impact.
\item \textbf{Quiz behavior:} retry until the player selects a valid answer.
\end{itemize}
...
\textbf{Intro:} Before hiring, the team tries to name the work that is currently hidden inside âgeneral coordination.â\\
...
\textbf{Intro:} The project now has to define what âgood staffingâ actually means.\\
...
\textbf{Main text:} A junior researcher quietly says they can no longer tell which concerns are âimportant enoughâ to raise because everything already feels urgent. That moment changes the chapter. Burnout is not only a wellbeing issue here. It is also a governance issue, because exhausted teams get worse at surfacing weak signals before those signals become expensive failures.\\
...
\textbf{Extra / note:} <h3>Reusable method</h3><p>Take one planned decision and ask who can meaningfully alter it before it is announced, budgeted, or framed as too expensive to revisit. If the answer is âalmost nobody,â the structure is probably thinner than it looks.</p>\\
...
\textbf{Main text:} A team is not only a list of competencies. It is a structure that decides which constraints become visible early, which ones stay outside the room, and how expensive it becomes to correct the project later. Many failures of âcommunicationâ are actually failures of team design: the right concern existed, but had no protected path into the decision.\\
...
\textbf{Extra / note:} <h3>Practical use</h3><p>This check is especially useful before hiring, before pilot design, and whenever a team says it will âadd broader perspectives later.â Later often means after the expensive choices are already made.</p>\\
...
\end{itemize}
\item \textbf{Choice 3:} Meetings are longer than they were during the projectâs earliest phase.
\begin{itemize}
\item \textbf{Impact:} No resource impact.
\item \textbf{Quiz behavior:} retry until the player selects a valid answer.
\end{itemize}
...
\textbf{Main text:} An early analysis suggests that the archive underrepresents some patient groups. The team now has to decide whether to keep pushing, pause for repair, or look for a faster compensating move that may ask a lot from the projectâs remaining trust.\\
\begin{itemize}
...
\textbf{Intro:} The team pauses long enough to write down what it would otherwise keep in peopleâs heads.\\
\textbf{Main text:} A short internal note captures the representation gaps, provenance uncertainties, and infrastructure costs that are now shaping the data conversation. It is not glamorous work, but it changes the chapterâs tone. Once these issues are written down in one place, it becomes harder for anyone to pretend they were minor side problems all along.\\
...
\textbf{Intro:} This information node names the chapterâs structural lesson.\\
...
\item \textbf{Quiz behavior:} retry until the player selects a valid answer.
\end{itemize}
\item \textbf{Choice 2:} The team can explain the archiveâs limits, known gaps, and reuse conditions without pretending they have disappeared.
\begin{itemize}
\item \textbf{Impact:} No resource impact.
...
\textbf{Main text:} The room is more grounded than earlier meetings: ward staff, project leads, and operational managers are all trying to imagine the same future week from different angles. It becomes clear very quickly that âlaunchâ is not one event. It is a chain of small handovers, ambiguities, and expectations that will either stay governable or become friction points as soon as the winter rush turns abstract confidence into practical strain.\\
...
\textbf{Main text:} A nurse manager walks through the pilot ward and points out several practical problems that no one had fully seen from the boardroom: alarm fatigue, handover timing, and how easily âdecision supportâ can still feel like pressure in a busy clinical setting. The quieter route suddenly feels less like caution in the abstract and more like respect for details that will decide whether staff can actually live with the tool.\\
...
\textbf{Main text:} A consultant physician asks a simple question in a tense tone: âWhen this system is wrong, who will be expected to carry that wrongness first?â The room falls quiet because everyone knows the answer cannot be technical alone.\\
...
\textbf{Main text:} The exercise is not technically sophisticated, but it is revealing. It shows who gets called first when the tool behaves oddly, where uncertainty is likely to be explained badly, and how quickly a âpilotâ can start feeling mandatory once pressure is high. Students often imagine that launch problems begin with system errors alone. Here the room sees that many of them begin with roles, expectations, and communication paths that were never made explicit enough.\\
...
\textbf{Intro:} The final quiz checks whether the whole gameâs logic has stayed intact.\\
...
\item \textbf{Quiz behavior:} retry until the player selects a valid answer.
\end{itemize}
\item \textbf{Choice 2:} A launch should match the projectâs actual balance of evidence, trust, feasibility, and correction capacity rather than only its ambition.
\begin{itemize}
\item \textbf{Impact:} No resource impact.
...
\textbf{Intro:} The chapter pauses one step before the ending to imagine the projectâs first week in practice.\\
